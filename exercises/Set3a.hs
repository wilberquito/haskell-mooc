...
firstHalf str = if oddStr str then take (mid + 1) str else take mid str
  where
    oddStr ls = odd $ length ls
    mid = length str `div` 2

palindrome :: String -> Bool
palindrome ls = all eq $ zip ls $ reverse ls
  where
    eq (x, y) = x == y
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) [k ^ e | e <- [0, 1 ..]]
...
joinToLength n ls = [str | a <- ls, b <- ls, let str = a ++ b, length str == n]
...
sumRights = sum . map zeroOrRight
  where
    zeroOrRight x = case x of
      (Left _) -> 0
      (Right b) -> b
