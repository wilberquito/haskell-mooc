...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) = Color (favg r1 r2) (favg g1 g2) (favg b1 b2)
...
paintSolid color (Shape f) (Picture g) = Picture f'
    where f' coord = if f coord then color else g coord
...
paint (Picture f) (Shape g) (Picture h) = Picture f'
    where f' coord = if g coord then f coord else h coord
------------------------------------------------------------------------------
...
instance Transform Fill where
  apply (Fill color) _ = Picture (\coor -> color)
