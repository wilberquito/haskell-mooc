...
setName name p = MkPerson {age = getAge p, name = name}

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = MkPerson {age = age, name = getName p}
...
data Position = Point Int Int deriving (Show)
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix c1 c2) = mix c1 c2
rgb (Invert c) = invert c
