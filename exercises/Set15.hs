...
statements xs ys = liftA2 (++) xs $ liftA2 (++) inter ys
    where
        inter = [" is ", " is not "]
...
calculator opstr x = op <*> (readMaybe x :: Maybe Int)
    where
        op
            | opstr == "negate" = Just (* (-1)) 
            | opstr == "double" = Just (* 2) 
...
validateAddress streetName streetNumber postCode = checkStreetName *> checkStreetNumber *> checkPostCode
  where checkStreetName = check (length streetName <= 20) "Invalid street name" address
        checkStreetNumber = check (all isDigit streetNumber) "Invalid street number" address
...
parseExpression exp =
    case words exp of
        [arg1, op, arg2] -> checkOperator op <*> checkArg arg1 <*> checkArg arg2
        _ -> invalid $ "Invalid expression: " ++ exp
    where
        checkOperator "+" = pure Plus
        checkOperator "-" = pure Minus
        checkOperator op = invalid $ "Unknown operator: " ++ op
        checkArg arg = checkNumber arg <|> checkVariable arg
        checkVariable var = check (length var == 1 && (isAlpha $ head var)) ("Invalid variable: " ++ var) (Variable $ head var)
        checkNumber num = check (isNumber num) ("Invalid number: " ++ num) (Number $ read num)
            where
                isNumber str = isJust (readMaybe str :: Maybe Int)
