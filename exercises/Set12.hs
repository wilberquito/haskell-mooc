...
instance Functor Result where
  ...
  fmap f (Failure message) = Failure message
...
inBoth fa fb = foldr (\e acc -> if e `elem` fb then e : acc else acc) [] fa
...
instance Foldable TwoList where
  foldr g acc TwoEmpty = acc
  foldr g acc (TwoNode x y xs) = g x $ g y (foldr g acc xs)
