...
rangeOf xs = abs $ maximum xs - minimum xs
...
longest :: Ord a => [[a]] -> [a]
longest xxs = smallestStart $ fst $ partition (\xs -> length xs == n) xxs
  where
    n = longestList xxs

longestList :: [[a]] -> Int
longestList = foldr (\xs x -> if x < length xs then length xs else x) 0

smallestStart :: Ord a => [[a]] -> [a]
smallestStart [] = []
...
winner scores player1 player2
  ...
  | otherwise = player1
  where
      p1Score = Map.findWithDefault 0 player1 scores
      p2Score = Map.findWithDefault 0 player2 scores

pPosition :: Eq a => a -> [(a, b)] -> Maybe Int
pPosition pName = findIndex (\(name,_) -> name == pName)
------------------------------------------------------------------------------
...
freqs = foldr (Map.alter add) Map.empty

add :: Num a => Maybe a -> Maybe a
add Nothing = Just 1
add (Just n) = Just (n + 1)
------------------------------------------------------------------------------
...
transfer from to amount bank
    ...
    | amount < 0 = bank
    | fromJust (Map.lookup from bank) - amount < 0 = bank
    | otherwise = Map.adjust (\c -> c - amount) from $ Map.adjust (+ amount) to bank
...
maxIndex arr = fst $ foldr (\x cand -> if snd x > snd cand then x else cand) x xs
    where 
        (x:xs) = assocs arr
