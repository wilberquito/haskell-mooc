...
appendAll r ls = forM_ ls (\s -> modifyIORef r (++ s))
...
hSelectLines h nums =
  do
    lines <- hFetchLines h
    let pickedLines = filter ((`elem` nums) . snd) [x | x <- zip lines [1 ..]]
    return (map fst pickedLines)
