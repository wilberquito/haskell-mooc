...
outOfBounds i ls = i < 0 || i >= length ls

updateAt :: Int -> a -> [a] -> [a]
updateAt _ _ [] = []
updateAt i x xs = if outOfBounds i xs then xs else updateAt' i x xs
...
palindrome [] = True
palindrome [x] = True
palindrome xs = (head xs == last xs) && palindrome (middle xs)
...
palindromify s
  | isPalindrome s = s
  | otherwise = palindromify (middle s)
...
greet first (Just name) = "Hello, " ++ first ++ " " ++ name ++ "!"
...
safeIndex xs i = if outOfBounds i xs then Nothing else Just (xs !! i)
...
eitherDiv x y = maybe (Left (show x ++ "/" ++ show y)) Right operation
  where
    operation = safeDiv x y
