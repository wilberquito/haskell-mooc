...
sums' n m ls
  | n == m = sumsIndex n : ls
  | otherwise = sumsIndex m : sums' n (m + 1) ls

sumsIndex :: Int -> Int
sumsIndex 0 = 0
sumsIndex n = n + sumsIndex (n - 1)
...
sumsOf :: [Int] -> [Int]
sumsOf xs = sumsOf' xs xs 0

sumsOf' :: [Int] -> [Int] -> Int -> [Int]
sumsOf' _ [] _ = []
sumsOf' orig (_ : xs) n = partialSum orig n : sumsOf' orig xs (n + 1)

partialSum :: [Int] -> Int -> Int
partialSum [] _ = 0
partialSum (x : xs) n
  | n < 0 = 0
  | otherwise = x + partialSum xs (n - 1)
...
mymaximum comp candidate [x] = if comp x candidate then x else candidate
mymaximum comp candidate (x : xs) = mymaximum comp (if comp x candidate then x else candidate) xs

------------------------------------------------------------------------------
-- Ex 9: define a version of map that takes two-argument a function
