...
maxHelper n m = if n > m then n else m
...
headHelper m Nothing = Just m
headHelper m (Just _) = Just m
...
lastHelper m Nothing = Just m
lastHelper _ (Just m) = Just m
